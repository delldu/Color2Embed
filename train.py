...
def train(
    ...
    for idx in pbar:
        ...
        ref_color_vector = colorEncoder(img_ref / 255.0)
        ...
        if get_rank() == 0:
            pbar.set_description((f"recon:{recon_val:.4f}; fea:{fea_val:.4f};"))
            ...
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    for inum in range(10):
                        ...
                        val_img_l = val_img_lab[:, :1, :, :] / 50.0  # [-1, 1]
                        # val_img_ref_ab = val_img_ref_lab[:,1:,:,:] / 110. # [-1, 1]

                        ref_color_vector = colorEncoder(val_img_ref / 255.0)  # [0, 1]
